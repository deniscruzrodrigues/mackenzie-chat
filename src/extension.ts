...
export function activate(context: vscode.ExtensionContext) {
	...
	if (vscode.window.registerWebviewViewProvider) {
		...
		context.subscriptions.push(
			vscode.window.registerWebviewViewProvider('mackenzie-chat.chatView',provider)
		);
	}
}
...
class ChatViewProvider implements vscode.WebviewViewProvider {
		...
		<body>
			<div class="chat-container">
				...
				<div class="chat-input-container">
					<ipnput type="text" id="chatinput" placeholder="Digite sua mensagem...">
	...
	private async _analyzeCurrentFile(webview: vscode.Webview) {
		...
		try {
			...
			const result = await this._callMcpTool('codeAnalzer', 'analyze_code', {
				file_path: filePath
			});
...
class ChatPanel {
	...
	private _getHtmlForWebview(webview: vscode.Webview, extensionUri: vscode.Uri) {
		//Código similar ao método _getHtmlForWebview da classe ChatViewProvider
		return "";
	}
